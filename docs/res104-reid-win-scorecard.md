...
**Status: `real-external-gold` (TAB). Single seed → directional; the #1 gap is large enough that its
Wilson CI excludes every competitor.** Companion: `runs/res104-reid-local.json`. Metric defined in
...
- **Single seed** (cjeu-structure seed 0); the #1 margin is large (CI excludes all others) but a
  multi-seed confirmation + a paired-bootstrap Δ CI would harden it to a citable claim.
