...
**Status: `real-external-gold` (TAB). Confirmed across 3 seeds — DIRECT-leak {0.0947, 0.0985, 0.0985},
mean 0.097, range 0.004; every seed beats the board-best (spacy 0.496) by ~5×.** Companion:
`runs/res104-reid-local.json`. Metric defined in
...
- **3-seed confirmed** (seeds 0/1/2): DIRECT-leak {0.0947, 0.0985, 0.0985}, mean 0.097, range 0.004 —
  ...
  Detection-F1 across seeds {0.340, 0.309, 0.317}. A formal paired-bootstrap Δ CI vs spacy is the only
  remaining step to a fully citable claim (the seed stability already settles robustness).
